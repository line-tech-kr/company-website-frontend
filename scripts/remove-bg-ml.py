...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Remove background from product photos."
    )
    parser.add_argument("skus", nargs="*", metavar="sku", help="SKU(s) to process")
    parser.add_argument(
        "--input",
        metavar="PATH",
        help="Explicit input image path (only valid with a single SKU)",
    )
    parser.add_argument(
        "--output",
        metavar="PATH",
        help="Explicit output path (only valid with a single SKU)",
    )
    ...
    if not args.skus:
        ...
        print("error: at least one SKU is required", file=sys.stderr)
        return 1

    if args.input and len(args.skus) > 1:
        print("error: --input can only be used with a single SKU", file=sys.stderr)
        return 1

    if args.output and len(args.skus) > 1:
        print("error: --output can only be used with a single SKU", file=sys.stderr)
        return 1
    ...
    for sku in args.skus:
        ...
        if args.input:
            src = Path(args.input)
            if not src.exists():
                print(f"  skip {sku}: --input path not found: {src}", file=sys.stderr)
                continue
        else:
        ...
        dst = Path(args.output) if args.output else PRODUCTS_DIR / sku / "cutout.png"
